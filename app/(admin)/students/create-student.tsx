...
export default function CreateStudentScreen() {
    ...
    async function handleCreateStudent() {
        ...
        try {
            setLoading(true);
            // Role hardcoded as "user" for students
            await register(name.trim(), email.trim(), username.trim(), password.trim(), "user");
            Alert.alert("Sucesso", "Estudante cadastrado com sucesso!");
            router.back();
        } catch (e: any) {
            Alert.alert("Erro", e.message || "Falha ao cadastrar estudante");
        } finally {
            setLoading(false);
        }
    }
    ...
    return (
        <Screen>
            <ScrollView contentContainerStyle={styles.page}>
                <Text style={styles.h1}>Cadastrar Estudante</Text>
                <Text style={styles.subtitle}>Crie uma nova conta de estudante</Text>
